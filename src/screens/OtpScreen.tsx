...
const onSubmit = useCallback(async () => {
  ...
  try {
    ...
    setSession({token, user}); // bunu cagırınca otpverifed true set ediyoruz ki ana menuye girebilelim. 
    toast.show(`Hoş geldin${user?.name ? `, ${user.name}` : ''}!`, 'success');
  } finally {
    setLoading(false);
  }
}, [filled, loading, setSession, currentUser]);
